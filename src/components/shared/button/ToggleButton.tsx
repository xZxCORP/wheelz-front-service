...
export const ToggleButton = ({ onClick, id, active, Icon }: Props) => {
  return (
    <Button
      onClick={() => onClick(id)}
      className={`${active ? 'bg-primary-500 pointer-events-none' : 'bg-primary-300'}  hover:bg-primary-500 active:bg-primary-500 size-14 text-black transition-transform hover:scale-105`}
    >
      <Icon size={25} />
    </Button>
  );
